package Seminars.Seminar5.ru.gb.jcore.Task1;
...
public class Main {
    ...
    private static final int CHAR_BOUND_L2 = 97; // Номер начального символа
    private static final int CHAR_BOUND_H = 90; // Номер конечного символа
    private static final int CHAR_BOUND_H2 = 122; // Номер конечного символа

    private static final String TO_SEARCH = "JavaCore"; // для поиска
...
//        }
    ...
    private static String generateSymbols(int amount) {
        ...
        for (int i = 0; i < amount; i++) {
            ...
            switch (test) {
                case 0 -> sb.append((char) random.nextInt(CHAR_BOUND_L, CHAR_BOUND_H + 1));
                case 1 -> sb.append((char) random.nextInt(CHAR_BOUND_L2, CHAR_BOUND_H2 + 1));
            }
        }
        return sb.toString();
    }
        ...
        try (FileInputStream fileInputStream = new FileInputStream(fileName)) {
            ...
            while ((c = fileInputStream.read()) != -1) {
                if (c == searchData[i]) {
                    ...
                    i = 0;
                    if (c == searchData[i]) // JavaCorJavacore
                        i++;
                    continue;
                }
